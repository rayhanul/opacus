...
def plot_epsilon_over_iterations(data_R2DP, data_Gaussian, title="epsilon_over_iterations"):
    ...
    print(len(gaussian))
    plt.figure(figsize=(10, 6))
    plt.plot(key, r2dp, '-b', label='Epsilon R2DP')
    plt.plot(key, gaussian, '-r', label='Epsilon Gaussian')
...
if __name__=="__main__":
    ...
    file_path=os.path.join(os.path.dirname(__file__), 'data_r2dp_dynamic0.pkl')

    with open(file_path, 'rb') as file:
        data_r2dp = pickle.load(file)

    file_path=os.path.join(os.path.dirname(__file__), 'data_gaussian_0.7.pkl')
    with open(file_path, 'rb') as file:
        data_gaussian = pickle.load(file)

    title="Accuracy over Epoch for Epsilon Gaussian=0.7"
